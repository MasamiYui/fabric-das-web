...
//        Criteria ownerCriteria = Criteria.where("title").regex(title).and("state");
    ...
    public JSONObject CheckVideoAssetAndChangeState(String id, String state) throws Exception {
        ...
        Query query = new Query();
        query.addCriteria(ownerCriteria);//条件查询
        Video video = mongoTemplate.find(query, Video.class).get(0);
        String selState = video.getState();
        ...
        HashMap<String, Object> dataMap = MapUtil.convertToMap(video);
        ...
        video.setState(state);
        video.setTransactionId(transactionId);
        try{
            mongoTemplate.save(video);
        }catch (Exception e){
            return ResponseUtil.constructResponse(400, "insert database failed.", null);//出现异常直接返回错误
