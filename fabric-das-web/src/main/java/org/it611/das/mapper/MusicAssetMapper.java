...
@Component
public interface MusicAssetMapper {

    //条件查询(以后可增加其他查询条件)
    List<HashMap> selectMusicAssetByConditions(@Param("title") String title);

    //查询记录数
    Long selectTotal(@Param("title") String title);

    HashMap<String, Object> selectMusicAssetDetailById(@Param("id") String id);
