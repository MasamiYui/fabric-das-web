...
public class DegreeCertificationAssetServiceImpl implements DegreeCertificationAssetService {
    ...
    public JSONObject CheckDegreeCertificationAndChangeState(String id, String state) throws Exception {
        ...
        String selState = degreeCertificate.getState();
        ...
        //如果是审核，即传入进来的state=1
        //获取mysql中的学位证书信息
        HashMap<String, Object> dataMap = MapUtil.convertToMap(degreeCertificate);
        ...
        degreeCertificate.setState(state);
        degreeCertificate.setTransactionId(transactionId);
        try{
            mongoTemplate.save(degreeCertificate);
        }catch (Exception e){
            return ResponseUtil.constructResponse(400, "insert database failed.", null);//出现异常直接返回错误
