...
public class PhotoAssetServiceImpl implements PhotoAssetService {
    ...
    public JSONObject photoAssetList(int currentPage, int numberOfPages, String title) {
        ...
        List<HashMap> result = photoAssetMapper.selectPhotoAssetByConditions(title);
        Long total = photoAssetMapper.selectTotal(title);
    ...
    public JSONObject photoAssetDetail(String id) throws InvalidArgumentException, NoSuchAlgorithmException, IOException, TransactionException, NoSuchProviderException, CryptoException, InvalidKeySpecException, ProposalException {
        ...
        HashMap<String, Object> mysqlDataMap = photoAssetMapper.selectPhotoAssetDetailById(id);//获取mysql中的数据
    ...
    public JSONObject CheckPhotoAssetAndChangeState(String id, String state) throws InvalidArgumentException, NoSuchAlgorithmException, IOException, TransactionException, NoSuchProviderException, CryptoException, InvalidKeySpecException, ProposalException {
        ...
        try {
            ...
            if(!transactionId.equals("")){//如果是已经上链的数据，即数据库中已经有transactionId，只需要改变状态
                int updateResult = photoAssetMapper.updateStateAndTransaction(id,transactionId, state);
                if (!(updateResult >0)) {
                    return ResponseUtil.constructResponse(400, "insert database failed.", null);//出现异常直接返回错误
                }
                return ResponseUtil.constructResponse(200, "ok", null);//出现异常直接返回错误
