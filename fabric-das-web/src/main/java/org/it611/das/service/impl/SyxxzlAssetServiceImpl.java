...
public class SyxxzlAssetServiceImpl implements SyxxzlAssetService {
    ...
    private static Logger logger= Logger.getLogger(SyxxzlAssetServiceImpl.class);
    ...
    @Override
    public JSONObject syxxzlList(int currentPage, int numberOfPages, String id,String state) {

        HashMap dataMap = new HashMap<String, Object>();
        Criteria ownerCriteria = null;
        if("".equals(state)){
            ownerCriteria = Criteria.where("id").regex(id);
        }else {
            ownerCriteria = Criteria.where("id").regex(id).and("state").is(state);
        }
    ...
    @Override
    @Transactional
    public JSONObject CheckSyxxzlAndChangeState(String id, String state) throws Exception {

        Criteria ownerCriteria = Criteria.where("id").is(id);
        Query query = new Query();
        query.addCriteria(ownerCriteria);//条件查询
        Syxxzl syxxzl = mongoTemplate.find(query, Syxxzl.class).get(0);
        String selState = syxxzl.getState();

        if(selState.equals(state))  return ResponseUtil.constructResponse(200, "ok", null);//状态相同直接返回

        //如果不是进行审核(不等于1)，即单纯的state更新
        if(!"1".equals(state)){
            syxxzl.setState(state);
            try{
                mongoTemplate.save(syxxzl);
            }catch (Exception e){
        ...
        //如果是审核，即传入进来的state=1
        //获取mysql中的学位证书信息
        HashMap<String, Object> dataMap = MapUtil.convertToMap(syxxzl);
        ...
        String syxxzlJsonStr = null;//value转json字符串
        try {
            syxxzlJsonStr = new ObjectMapper().writeValueAsString(dataMap);
            //执行Fabric的写入（数据上链）
            ChaincodeManager manager = FabricManager.obtain().getManager();
            String[] arguments = new String[]{assetId, syxxzlJsonStr};
        ...
        syxxzl.setState(state);
        syxxzl.setTransactionId(transactionId);
        try{
            mongoTemplate.save(syxxzl);
        }catch (Exception e){
            return ResponseUtil.constructResponse(400, "insert database failed.", null);//出现异常直接返回错误
